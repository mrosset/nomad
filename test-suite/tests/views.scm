...
(define-method (test-views-routes (self <test-views>))
  (assert-true (match-route "" %routes))
  (assert-true (match-route "/file/path" %routes))
  (assert-equal 2 (length %routes))
  (add-route! "^/test" #t)
  (assert-equal 3 (length %routes))
  (assert-true (car (list-ref %routes 2)))
  (assert-true (match-route "/test/some/path" %routes))
  (assert-false (match-route "/not/a/match" %routes)))
