...
#define VTE_PRIVATE(o) \
	(G_TYPE_INSTANCE_GET_PRIVATE ((o), WEMACS_TYPE_VTE, WemacsVtePrivate))
...
wemacs_vte_class_init (WemacsVteClass * class)
{
	...
	g_type_class_add_private (class, sizeof (WemacsVtePrivate));
}
...
wemacs_vte_init (WemacsVte * self)
{
	...
	vte_terminal_set_color_background (VTE_TERMINAL (priv), &b_rgba);
	vte_terminal_set_color_foreground (VTE_TERMINAL (priv), &f_rgba);

	fork_vte_child (VTE_TERMINAL (priv), 0, NULL);
	g_signal_connect (priv, "child-exited", G_CALLBACK (fork_vte_child), NULL);
}
