...
msg (void *data)
{
  ...
  SCM addr = scm_from_locale_string ("/tmp/wemacs");
  SCM message = scm_string_to_utf8 (scm_from_locale_string ("(version)"));
  //  scm_connect (sock, scm_from_int(AF_UNIX), scm_from_locale_string("/tmp/wemacs"), SCM_EOL);
  //r  scm_sendto (sock, message, fam, addr, 0);
}
